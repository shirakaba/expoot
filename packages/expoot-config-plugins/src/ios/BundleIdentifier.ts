...
export const withBundleIdentifier: ConfigPlugin<{
...
function getProductBundleIdentifierFromBuildConfiguration(
  ...
  const bundleIdentifierRaw =
    xcBuildConfiguration.buildSettings.PRODUCT_BUNDLE_IDENTIFIER;
  if (bundleIdentifierRaw) {
    const bundleIdentifier = trimQuotes(bundleIdentifierRaw);
    return resolveXcodeBuildSetting(
      bundleIdentifier,
      (setting) =>
        xcBuildConfiguration.buildSettings[setting] as string | undefined
    );
  } else {
    return null;
  }
