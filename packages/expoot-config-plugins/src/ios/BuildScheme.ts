import { readXMLAsync } from '@expo/config-plugins/build/utils/XML';
...
async function readSchemeAsync(
  ...
  if (schemePath) {
    return (await readXMLAsync({ path: schemePath })) as unknown as
      | SchemeXML
      | undefined;
  } else {
    throw new Error(
      `scheme '${scheme}' does not exist, make sure it's marked as shared`
    );
  }
}
...
export async function getApplicationTargetNameForSchemeAsync(
  ...
  const buildActionEntry =
    schemeXML?.Scheme?.BuildAction?.[0]?.BuildActionEntries?.[0]
      ?.BuildActionEntry;
  const targetName =
    buildActionEntry?.length === 1
      ...
      : getBlueprintName(
          buildActionEntry?.find((entry) => {
            return entry.BuildableReference?.[0]?.[
              '$'
            ]?.BuildableName?.endsWith('.app');
          })
        );
  if (!targetName) {
...
export async function getArchiveBuildConfigurationForSchemeAsync(
  ...
  const schemeXML = await readSchemeAsync(projectRoot, platform, scheme);
  const buildConfiguration =
    schemeXML?.Scheme?.ArchiveAction?.[0]?.['$']?.buildConfiguration;
