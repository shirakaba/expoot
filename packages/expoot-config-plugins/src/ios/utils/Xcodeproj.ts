...
function createProjectFileForGroup({
  ...
  const conflictingFile = group.children.find(
    (child) => child.comment === file.basename
  );
  if (conflictingFile) {
...
export function addFileToGroupAndLink({
  ...
  if (targetUuid != null) {
    ...
    const applicationNativeTarget = project.getTarget(
...
function pbxGroupByPathOrAssert(project: XcodeProject, path: string): PBXGroup {
  ...
  if (!group) {
    throw Error(
      `Xcode PBXGroup with name "${path}" could not be found in the Xcode project.`
    );
  }
...
export function ensureGroupRecursively(
  project: XcodeProject,
  filepath: string
): PBXGroup | null {
...
export function getXCConfigurationListEntries(
  project: XcodeProject
...
export function getBuildConfigurationsForListId(
  ...
  return Object.entries(project.pbxXCBuildConfigurationSection())
    ...
    .filter(([key]: ConfigurationSectionEntry) =>
...
export function isBuildConfig([
  ,
  sectionItem,
]: ConfigurationSectionEntry): boolean {
  return sectionItem.isa === 'XCBuildConfiguration';
}

export function isNotTestHost([
  ,
  sectionItem,
]: ConfigurationSectionEntry): boolean {
