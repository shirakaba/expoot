...
export function withBaseMod<T>(
  ...
  if (!(config.mods as any)[platform]) {
    (config.mods as any)[platform] = {};
  }

  let interceptedMod: Mod<T> = (
    (config.mods as any)[platform] as Record<string, any>
  )[mod];
  ...
  async function interceptingMod({
    ...
    if (saveToInternal) {
      saveToInternalObject(
        results,
        platform,
        mod,
        results.modResults as unknown as JSONObject
      );
    }
    return results;
  }
  ...
  (config.mods as any)[platform][mod] = interceptingMod;
...
export function withMod<T>(
  ...
  return withBaseMod(config, {
    ...
    isProvider: false,
    async action({
      modRequest: { nextMod, ...modRequest },
      modResults,
      ...config
    }) {
      const results = await action({
        modRequest,
        modResults: modResults as T,
        ...config,
      });
