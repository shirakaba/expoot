import { get } from '@expo/cli/build/src/utils/obj.js';
import { getConfig, getProjectConfigDescriptionWithPaths } from '@expo/config';
import type { ModPlatform } from '@expoot/config-plugins';
...
export class AppIdResolver {
  ...
  async getAppIdFromConfigAsync(): Promise<string> {
    ...
    const appId = get(config.exp as ExpoConfig, this.configProperty);
    if (!appId) {
