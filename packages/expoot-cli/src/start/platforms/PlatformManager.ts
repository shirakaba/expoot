...
import { learnMore } from '@expo/cli/src/utils/link.js';
...
const debug = Debug(
  'expo:start:platforms:platformManager'
...
export class PlatformManager<
  ...
  IResolveDeviceProps extends
    BaseResolveDeviceProps<IDevice> = BaseResolveDeviceProps<IDevice>,
> {
  constructor(
  ...
  protected async getExpoGoOrCustomRuntimeUrlAsync(
    ...
    if (redirectUrl) {
      ...
      if (applicationId) {
        debug(
          `Resolving launch URL: (appId: ${applicationId}, redirect URL: ${redirectUrl})`
        );
        ...
        if (
          await deviceManager.isAppInstalledAndIfSoReturnContainerPathForIOSAsync(
            applicationId
          )
        ) {
          return redirectUrl;
  ...
  protected async openProjectInCustomRuntimeAsync(
    ...
    const applicationId =
      props.applicationId ?? (await this._getAppIdResolver().getAppIdAsync());
  ...
  async openAsync(
    ...
    if (options.runtime === 'expo') {
      ...
      return this.openProjectInCustomRuntimeAsync(
        resolveSettings,
        options.props
      );
    } else {
  ...
  private async openWebProjectAsync(
    resolveSettings: Partial<IResolveDeviceProps> = {}
  ): Promise<{
    url: string;
  }> {
