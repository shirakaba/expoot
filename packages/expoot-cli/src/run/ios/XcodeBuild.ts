...
import { ensureDirectory } from '@expo/cli/build/src/utils/dir.js';
import { getUserTerminal } from '@expo/cli/build/src/utils/terminal.js';
...
export function matchEstimatedBinaryPath(buildOutput: string): string | null {
  ...
  if (!appBinaryPathMatch?.length) {
    ...
    const shortestPath = appBinaryPathMatch
...
export function getAppBinaryPath(buildOutput: string) {
  ...
  try {
    ...
    const binaryPath = path.join(
      ...
      UNLOCALIZED_RESOURCES_FOLDER_PATH[
        UNLOCALIZED_RESOURCES_FOLDER_PATH.length - 1
      ]
    );
...
export async function getXcodeBuildArgsAsync(
  ...
  if (
    !props.isSimulator ||
    simulatorBuildRequiresCodeSigning(props.projectRoot, props.platform)
  ) {
...
function spawnXcodeBuild(
  ...
  buildProcess.stderr.on('data', (data: Buffer) => {
    const stringData =
      data instanceof Buffer ? data.toString() : (data as unknown as string);
    error += stringData;
  });
...
async function spawnXcodeBuildWithFormat(
  ...
  options: SpawnOptionsWithoutStdio,
  {
    projectRoot,
    xcodeProject,
  }: { projectRoot: string; xcodeProject: ProjectInfo }
): Promise<{
  code: number | null;
  results: string;
  error: string;
...
function getErrorLogFilePath(projectRoot: string): [string, string] {
  ...
  return [
    path.join(folder, 'xcodebuild.log'),
    path.join(folder, 'xcodebuild-error.log'),
  ];
}
