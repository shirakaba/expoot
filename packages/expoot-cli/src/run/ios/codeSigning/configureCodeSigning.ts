import * as Security from '@expo/cli/build/src/run/ios/codeSigning/Security.js';
import { resolveCertificateSigningIdentityAsync } from '@expo/cli/build/src/run/ios/codeSigning/resolveCertificateSigningIdentity.js';
...
function isCodeSigningConfigured(
  ...
  const allTargetsHaveTeams = Object.values(signingInfo).reduce(
    (prev, curr) => {
  ...
  if (allTargetsHaveTeams) {
    const teamList = Object.values(signingInfo).reduce<string[]>(
      (prev, curr) => {
    ...
    Log.log(
      chalk.dim`\u203A Auto signing app using team(s): ${teamList.join(', ')}`
    );
    return true;
  }

  const allTargetsHaveProfiles = Object.values(signingInfo).reduce(
    (prev, curr) => {
...
async function configureCodeSigningAsync(
  projectRoot: string,
  platform: ModPlatform
) {
