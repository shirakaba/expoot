...
import { intersecting } from '@expo/cli/build/src/utils/array.js';
import { getConfig } from '@expo/config';
import plist from '@expo/plist';
import { AndroidConfig, IOSConfig, ModPlatform } from '@expoot/config-plugins';
...
const debug = Debug('expo:utils:scheme') as typeof console.log;
...
function resolveExpoOrLongestScheme(schemes: string[]): string[] {
  ...
  return expoOnlySchemes.length > 0
    ? sortLongest(expoOnlySchemes)
    : sortLongest(schemes);
}
...
export async function getSchemesForIosAsync(
  ...
  try {
    const infoPlistBuildProperty = IOSConfig.getInfoPlistPathFromPbxproj(
      projectRoot,
      platform
    );
    ...
    debug(
      'expected error collecting ios application schemes for the main target:',
      error
    );
  }
...
export async function getSchemesForAndroidAsync(
  ...
  try {
    ...
    const manifest =
      await AndroidConfig.Manifest.readAndroidManifestAsync(configPath);
    const schemes = await AndroidConfig.Scheme.getSchemesFromManifest(manifest);
    debug('android application schemes:', schemes);
    return resolveExpoOrLongestScheme(schemes);
  } catch (error) {
    debug(
      'expected error collecting android application schemes for the main activity:',
      error
    );
...
// TODO: Revisit and test after run code is merged.
async function getManagedDevClientSchemeAsync(
  projectRoot: string
): Promise<string | null> {
  ...
  try {
    const getDefaultScheme = require(
      resolveFrom(projectRoot, 'expo-dev-client/getDefaultScheme')
    );
    const scheme = getDefaultScheme(exp as ExpoConfig);
    return scheme;
  } catch {
    Log.warn(
...
export async function getOptionalDevClientSchemeAsync(
  ...
  scheme: string | null;
  resolution: 'config' | 'shared' | 'android' | 'ios';
}> {
